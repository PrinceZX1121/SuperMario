    ...
    public Map() throws IOException {
        ...
        for (i = 0; i < width; i++) {
            for (j = 0; j < height; j++) {
                ...
                switch (argb) {
                    case 0xff000000 -> mapBlockInfo[i][j] = new Floor(i*32, j*32);
                    case 0xffffffff -> mapBlockInfo[i][j] = new Air(i*32, j*32);
                    case 0xffff0000 -> mapBlockInfo[i][j] = new Brick(i*32, j*32);
                    case 0xffffff00 -> mapBlockInfo[i][j] = new Query(i*32, j*32);
                }
                /*
                黑色：0xff000000   floor
                白色：0xffffffff   air
                ...
                this.blockList.add(mapBlockInfo[i][j]);
            }
        }
        ...
        for (i = 0; i < width; i++) {
            for (j = 0; j < height; j++) {
                mapGraphics.drawImage(mapBlockInfo[i][j].getImage(), i * 32, j * 32, null);
            }
        }
